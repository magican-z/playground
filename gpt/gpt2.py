...
class DataLoaderSuper():
    ...
    def reset(self):
        if self.curr_file_idx + 1 >= len(self.data_files):
            if self.shuffle:
                random.shuffle(self.data_files)
...
dl = DataLoaderSuper('./fineweb-edu-tokens', B, T, ddp_world_size, ddp_local_rank, 'train', shuffle=True)
if restore_checkpoint_path:
    dl.restore(checkpoint['data_state'])
...
run_steps = max_steps
for i in range(start_step, run_steps):
    t0 = time.time()
    optimizer.zero_grad()
    loss_accum = 0.0
    ...
    norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0) # norm的性能损耗很大
    lr = get_lr(i)
    ...
    processed_tokens = B * T * grad_accum_steps * ddp_world_size
    tokens_per_sec = processed_tokens / dt * 1000
    if master_process:
        print(f'step: {i} loss:{loss_accum.item():.6f} | cost:{dt:.2f}ms | tokens/sec: {tokens_per_sec:.2f} | norm:{norm:.4f} | lr:{lr:.4e}')
        ...
        if i > 0 and i % 1000 == 0 or i == run_steps - 1:
            checkpoint_path = os.path.join(f'./model_{i:05d}.pt')
            checkpoint = {
